...
function init() {
    ...
    // alert(b1);
    //b1 클릭시 News.json 데이터 가져와 result에 출력하세요
    ...
    function init() {
        ...
        b4.onclick = async () => {
            ...
            const proxy_url =
                'https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fwww.hankyung.com%2Ffeed%2Feconomy&api_key=ynbtsdcphivcqunydusqnj9zoe90tjjc9zzinhwu';
    ...
    b3.onclick = async () => {
        ...
        //한경닷컴 서버로 직접 요청을 보내보자. ==> 에러 발생 (sop 정책 때문)
        //same origin policy (동일 출처 정책) : 같은 서버 내에서만 통신 가능
        //cors 정책(cross origin resource sharing)에 의해 차단됨
    ...
    b4.onclick = async () => {
        const url = 'https://www.hankyung.com/feed/it';
        //대리서버(proxy server-rss2json)를 통해 요청을 보내자
        const proxy_url =
            'https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fwww.hankyung.com%2Ffeed%2Fit&api_key=ynbtsdcphivcqunydusqnj9zoe90tjjc9zzinhwu';
